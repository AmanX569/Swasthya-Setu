...
(function(global) {
  ...
  class AdminController {
    ...
    syncLiveFirebaseDatabase() {
      ...
      // 1. Initial REST Hydration for all Admin tables
      fetch(`${dbUrl}/.json`)
        .then(r => r.json())
        .then(cloudData => {
          if (cloudData && typeof cloudData === 'object') {
            ...
            if (cloudData.rural_inventory) {
              ...
              if (items.length) this.data.inventory = items;
            }
            if (cloudData.hospital_bed_grid) {
              const beds = Array.isArray(cloudData.hospital_bed_grid) ? cloudData.hospital_bed_grid : Object.values(cloudData.hospital_bed_grid);
              if (beds.length) this.data.hospitalGrid = beds;
            }
            if (cloudData.blood_bank_units) {
              const blood = Array.isArray(cloudData.blood_bank_units) ? cloudData.blood_bank_units : Object.values(cloudData.blood_bank_units);
              if (blood.length) this.data.bloodBank = blood;
            }
            if (cloudData.worker_assignments) {
              const workers = Array.isArray(cloudData.worker_assignments) ? cloudData.worker_assignments : Object.values(cloudData.worker_assignments);
              if (workers.length) this.data.workerAssignments = workers;
            }
      ...
      if (window.firebaseConfigManager && window.firebaseConfigManager.rtdb) {
        ...
        rtdb.ref('rural_inventory').on('value', snap => {
          const val = snap.val();
          if (val) {
            this.data.inventory = Array.isArray(val) ? val : Object.values(val);
    ...
    async openReassignModal(assignmentId) {
      ...
      // Sync to Firebase
      try {
        fetch(`https://swasthya-setu-2b67d-default-rtdb.firebaseio.com/worker_assignments/${a.id}.json`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(a)
        }).catch(() => {});
        if (window.firebaseConfigManager && window.firebaseConfigManager.rtdb) {
          window.firebaseConfigManager.rtdb.ref(`worker_assignments/${a.id}`).set(a);
        }
      } catch (err) {}

      if (typeof window.toast === 'function') {
        window.toast(`✓ Route for ${a.workerName} updated & synced to Firebase!`);
      }
    ...
    async reorderMedicine(medId) {
      ...
      med.lastRestocked = 'Today (Restocked)';

      // Sync to Firebase
      try {
        fetch(`https://swasthya-setu-2b67d-default-rtdb.firebaseio.com/rural_inventory/${med.id}.json`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(med)
        }).catch(() => {});
        if (window.firebaseConfigManager && window.firebaseConfigManager.rtdb) {
          window.firebaseConfigManager.rtdb.ref(`rural_inventory/${med.id}`).set(med);
        }
      } catch (err) {}

      if (typeof window.toast === 'function') {
        window.toast(`✓ Replenishment of ${qty} units synced to Firebase for ${med.name}!`);
      }
            ...
            <div class="glass-panel admin-hosp-card">
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;margin-top:16px;padding-top:12px;border-top:1px solid rgba(220,252,243,0.1);">
                ...
                <div style="display:flex;gap:6px;">
                  <button class="btn-glass sm" style="color:#fca5a5;" onclick="adminController.allocateHospitalBed('${h.id}', 'admit')">
                    <span>+1 Admit Patient</span>
                  </button>
                  <button class="btn-glass sm" style="color:#86efac;" onclick="adminController.allocateHospitalBed('${h.id}', 'discharge')">
                    <span>-1 Free Bed</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div class="admin-blood-full-grid">
          ${blood.map(b => `
            <div class="glass-panel admin-blood-card ${b.statusType}">
              ...
              <div style="display:flex;gap:6px;">
                <button class="btn-glass sm" style="flex:1;justify-content:center;" onclick="adminController.broadcastBloodAppeal('${b.group}')">
                  <span>📢 Request</span>
                </button>
                <button class="btn-glass sm" style="color:#86efac;padding:4px 10px;" onclick="adminController.updateBloodBankUnits('${b.group}', 5)" title="Add +5 Units">
                  <span>+5 Units</span>
                </button>
              </div>
            </div>
    ...
    async allocateHospitalBed(hospId, action = 'admit') {
      const hosp = this.data.hospitalGrid.find(h => h.id === hospId);
      if (!hosp) return;

      if (action === 'admit') {
        if (hosp.availableBeds > 0) {
          hosp.availableBeds -= 1;
          hosp.occupiedBeds += 1;
        } else {
          alert('No beds available in this hospital!');
          return;
        }
      } else if (action === 'discharge') {
        if (hosp.occupiedBeds > 0) {
          hosp.availableBeds += 1;
          hosp.occupiedBeds -= 1;
        }
      }

      hosp.occupancyRate = Math.round((hosp.occupiedBeds / hosp.totalBeds) * 100);
      ...
      try {
        ...
        if (window.firebaseConfigManager && window.firebaseConfigManager.rtdb) {
          window.firebaseConfigManager.rtdb.ref(`hospital_bed_grid/${hosp.id}`).set(hosp);
        }
      } catch (err) {}

      if (typeof window.toast === 'function') {
        window.toast(`✓ Bed Grid updated for ${hosp.name.split('(')[0]} (${hosp.availableBeds} free) & synced to Firebase!`);
      }

      this.renderHospitalGrid();
    }

    async updateBloodBankUnits(group, delta = 5) {
      const item = this.data.bloodBank.find(b => b.group === group);
      if (!item) return;

      item.units += delta;
      if (item.units >= item.minimum) {
        item.status = 'Adequate Stock';
      ...
      try {
        ...
        if (window.firebaseConfigManager && window.firebaseConfigManager.rtdb) {
          window.firebaseConfigManager.rtdb.ref(`blood_bank_units/${group}`).set(item);
        }
      } catch (err) {}

      if (typeof window.toast === 'function') {
        window.toast(`✓ ${delta > 0 ? '+' : ''}${delta} Units updated for ${group} blood & synced to Firebase!`);
      }
