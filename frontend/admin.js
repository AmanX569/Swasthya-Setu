  ...
  class AdminController {
    ...
    renderStaffManagement() {
      ...
      container.innerHTML = `
        ...
        <!-- Inline Instant Add Member Bar (Always Visible & 100% Reliable) -->
        <div class="glass-panel" style="padding:16px 20px;margin-top:14px;background:rgba(16,185,129,0.06);border:1.5px solid var(--auth-primary-bright);border-radius:16px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
            <div style="display:flex;align-items:center;gap:8px;">
              <span style="font-size:18px;">⚡</span>
              <strong style="font-size:14.5px;color:#ffffff;">Add New Staff or Citizen Member</strong>
            </div>
            <span class="admin-status-badge good">Live Cloud Realtime Sync</span>
          </div>
          <form onsubmit="adminController.submitQuickAddStaff(event)" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;">
            <div style="flex:1.5;min-width:160px;">
              <label class="auth-label" style="font-size:11px;margin-bottom:4px;display:block;">Full Name</label>
              <input type="text" id="quickStaffName" class="auth-input" placeholder="e.g. Dr. Rajesh / Priya Sharma" required autocomplete="off" style="height:42px;">
            </div>
            <div style="flex:1;min-width:130px;">
              <label class="auth-label" style="font-size:11px;margin-bottom:4px;display:block;">Role</label>
              <select id="quickStaffRole" class="auth-input" required style="height:42px;">
            ...
            <div style="flex:1.2;min-width:130px;">
              <label class="auth-label" style="font-size:11px;margin-bottom:4px;display:block;">Mobile Number</label>
              <input type="tel" id="quickStaffPhone" class="auth-input" placeholder="e.g. 9876543210" required autocomplete="off" style="height:42px;">
            </div>
            <div style="flex:1.2;min-width:140px;">
              <label class="auth-label" style="font-size:11px;margin-bottom:4px;display:block;">Facility / Ward</label>
              <input type="text" id="quickStaffFacility" class="auth-input" placeholder="Kondapalli PHC" value="Kondapalli Grid" style="height:42px;">
            </div>
            <button type="submit" class="auth-btn-primary" style="height:42px;padding:0 22px;font-size:13.5px;white-space:nowrap;justify-content:center;">
    ...
    submitQuickAddStaff(event) {
      if (event && event.preventDefault) event.preventDefault();

      const nameEl = document.getElementById('quickStaffName');
      const roleEl = document.getElementById('quickStaffRole');
      const phoneEl = document.getElementById('quickStaffPhone');
      const facilityEl = document.getElementById('quickStaffFacility');

      const name = nameEl ? nameEl.value.trim() : '';
      const role = roleEl ? roleEl.value : 'worker';
      ...
      // 3. Cloud Sync (Non-blocking background)
      try {
        fetch(`https://swasthya-setu-2b67d-default-rtdb.firebaseio.com/staff_registry/${uniqueId}.json`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (typeof window.toast === 'function') {
        window.toast(`✓ Added ${name} (${role.toUpperCase()}) to live roster & Cloud!`);
      }
    ...
    openAddStaffModal() {
      ...
      if (modal) {
        ...
        const name = prompt('Full Name of Member:');
        if (!name) return;
        const role = prompt('Role (doctor, worker, admin, patient):', 'doctor') || 'doctor';
        const phone = prompt('Mobile Number:', '9876543210') || '9876543210';
        const facility = prompt('Facility / PHC Location:', 'Kondapalli PHC') || 'Kondapalli Grid';
        
        const uniqueId = `USR-${role.toUpperCase().slice(0,3)}-${Date.now().toString().slice(-4)}`;
        const newMember = { id: uniqueId, userId: uniqueId, name, role, roles: [role], phone, facility, status: 'Active', regDate: 'Today', isNew: true, verified: true };
        this.data.users.unshift(newMember);
        try {
          fetch(`https://swasthya-setu-2b67d-default-rtdb.firebaseio.com/staff_registry/${uniqueId}.json`, {
            method: 'PUT',
        ...
        if (typeof window.toast === 'function') window.toast(`✓ Added ${name} to Cloud Database!`);
        this.calculateLiveStats();
        this.renderStaffManagement();
      }
    }
