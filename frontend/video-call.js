/**
 * =========================================================
 * SWASTHYA SETU - REAL-TIME WEBRTC VIDEO TELECONSULTATION ENGINE (video-call.js)
 * Peer-to-Peer Video Streams, Doctor Ringing Alert & In-Call e-Prescription
 * =========================================================
 */
...
(function(global) {
  ...
  class VideoCallController {
    ...
    handleIncomingSignal(signal) {
      ...
      if (signal.type === 'CALL_INITIATED') {
        if (activeRole === 'doctor') {
          // Check if targeted to this doctor or general on-duty pool
      ...
      // 4. ICE Candidates exchange
        ...
        if (this.peerConnection && this.currentCallData && this.currentCallData.id === signal.callId) {
          if (signal.candidate) {
            try {
              this.peerConnection.addIceCandidate(new RTCIceCandidate(signal.candidate)).catch(() => {});
            } catch (e) {}
          }
        }
      }
    ...
    // -------------------------------------------------------------
    // 2. PATIENT INITIATES CALL (OFFER)
    // -------------------------------------------------------------
    async startVideoCall(callDetails = {}) {
      ...
      if (global.toast) {
        global.toast('📞 Calling ' + recipientName + '... Please wait for doctor to accept.');
      }
    }
    ...
    declineIncomingCall() {
      ...
      if (this.pendingIncomingSignal && global.supabaseService) {
        global.supabaseService.sendTeleconsultSignal({
          ...
          reason: 'Doctor is currently occupied in emergency OPD'
        });
      }
      this.pendingIncomingSignal = null;
    }
    ...
    async acceptIncomingCall() {
      ...
      // Handle Patient Offer & Generate Answer
      let answerSdp = null;
      try {
        if (signal.offer) {
    ...
    setupPeerConnection() {
      try {
        ...
        // On Remote Track Received -> Attach to Remote Video Element
        ...
        this.peerConnection.onicecandidate = (event) => {
          if (event.candidate && this.currentCallData && global.supabaseService) {
            global.supabaseService.sendTeleconsultSignal({
              type: 'ICE_CANDIDATE',
              callId: this.currentCallData.id,
        ...
        console.warn('[WebRTC] RTCPeerConnection initialization:', err.message);
      }
    }
