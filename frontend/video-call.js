/**
 * =========================================================
 * SWASTHYA SETU - BULLETPROOF WEBRTC AUDIO & HD VIDEO ENGINE (video-call.js)
 * Web Audio Hardware Routing, Explicit Transceivers, Live Mic Meter & 720p HD
 * =========================================================
 */
  ...
  class VideoCallController {
    ...
    setupPeerConnection() {
      try {
        ...
        // Add explicit transceivers for bidirectional audio & video
        if (typeof this.peerConnection.addTransceiver === 'function') {
          this.peerConnection.addTransceiver('audio', { direction: 'sendrecv' });
          this.peerConnection.addTransceiver('video', { direction: 'sendrecv' });
        }

        // Add Local Audio and Video Tracks directly to Peer
        if (this.localStream) {
        ...
        this.peerConnection.ontrack = (event) => {
          ...
          // 3. Direct Web Audio Context Hardware Routing (Bypasses Autoplay Restrictions)
          if (event.track.kind === 'audio') {
            try {
              ...
              if (AudioCtx) {
                ...
                console.log('[WebRTC Audio] Remote voice stream connected directly to device speakers via AudioContext!');
              }
            } catch (err) {
      ...
      try {
        ...
        this.micMeterInterval = setInterval(() => {
          if (this.audioAnalyser && meterBar) {
            ...
            meterBar.style.background = this.isAudioMuted ? '#dc2626' : '#22c55e';
          }
        }, 150);
      } catch (e) {
